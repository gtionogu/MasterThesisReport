...
It aims similarly at the same as the normalization but through another method. The other reason standardization is applied to features is because some of the models improve their learning when re-scaling is applied.
\\
...
A summary of our optimization for the machine learning part of the project would be to find the best combination of classifier, scaling function, feature selection and hyper-parameter tuning.

\subsubsection{Analysis of the model's prediction abilities}
Finally, we need metrics that will provide us information on the experiment to enable the feedback loop. A very common way to do so is using the confusion matrix which we can derive a large number of useful metrics including accuracy, precision, recall, F1-Score. There are also other advanced metrics \cite{ml-metrics} that can be introduced as well such as: Logarithmic Loss, Area Under ROC Curve and Classification Report. All will be described in this section but will be detailed in the experiment section.
\paragraph{Logarithmic Loss}
\paragraph{Area Under ROC Curve}
\paragraph{•}
