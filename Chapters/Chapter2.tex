...
\paragraph{What is the concept of IP/Fast-flux networks?} IP-flux does the following: Associate a certain number of IP addresses to a single FQDN. One of these addresses is then presented using the round-robin algorithm when requests are made for the main FQDN. This technique is normally used for load distribution, load balancing or fault-tolerance. All these addresses potentially host the identical servers, round-robin simply decides on the order it will present them when a request is made for the FQDN.\cite{robin_dns} Round-robin slices time into small periods and presents equal blocks of these addresses in a circular order, it doesn't provide priority for any of the blocks of addresses or specific addresses.\cite{robin}\\
Fast-Flux on the other hand, uses IP-fluxing to hide a dynamic network of malicious proxies. \cite{wiki_ff}

IMHERE kinda

\paragraph{What was the idea behind this feature ?} The goal was to enable IP-fluxing for Content Delivery Networks(CDN) to be able to point customers towards other nodes in the network to obtain the content sought. \\
Botmasters use it to distribute a DNS request to a large number of compromised computers . The Fast-flux Service Network (FFSN) motherships are the controlling parts of the networks. They are very similar to the command and a control (CnC) system found in conventional botnets but provide more features. It is observed that these nodes are always hosting both DNS and HTTP services, for being able to manage the content availability for thousands of domains simultaneously on a single host. They collect information on the IP addresses assigned to the domain name and how those IP addresses (A and NS records) change over time.\cite{bot-com}
\cite{detection3}
...
\\
The power of Fast-Flux Service Network (FFSN) is allowing one domain name to have an unlimited number of IP addresses. The IP addresses belonging to such a domain act as a proxy for any device attempting a connection with their respective CnC server. This process helps botnet controllers avoid detection and blacklisting. Attackers have developed better techniques utilizing IP-flux over time, here are the different categories:
\begin{itemize}[noitemsep]
...
WIKIPEDIA: "single-flux", is characterized by multiple individual nodes within the network registering and de-registering their addresses as part of the DNS A (address) record list for a single DNS name.
...
DNS tunneling implies the act of tunneling other protocols or data through DNS, for example using the TXT as q\_type and using the requests to actually send data. This has been done to avoid restrictions but is now also used to hide malicious traffic from botnets. They can also use DNS tunneling to remain undetected while exfiltrating data. \cite{Botnet1}
...
- host based behavior: observable calls on the systems infected by botnets. 
