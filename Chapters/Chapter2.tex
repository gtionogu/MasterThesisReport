...
Antonakakis et al. propose a different system called 'Notos' based on dynamic reputation od domain names. They studied the different aspects around the historical DNS data of domains that could be relevant to group domains based on their legitimacy. They use 3 categories of features, network, zone and evidence. %%%%% USE z-thesis



 based on which they used as input for a reputation function that provides a score for each domain and classifies it.
What is interesting is that they showed that these features worked in a small environment as well which will compliment the group of features we'll use in our research.
