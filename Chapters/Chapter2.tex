...
There are three mains categories of algorithms in machine learning: supervised learning, unsupervised learning and semi-supervised learning.\\
These categories are based on the training data used for the algorithms. Supervised algorithms are provided labeled data, they analyze the features of the labels given and learn what characteristics are proper to each label. This is how the classifiers are build to predict labels for new data.\\
Unsupervised algorithms use unlabeled data for training. The algorithm usually provides clustering of the data, predicting which cluster new data belongs to.\\
Semi-supervised algorithms is the combination of both, where the training data is partially labeled.

\paragraph{why use machine learning for botnet detection?}
Classification algorithms are very effective when it comes to detection problems, predictions, filtering.
