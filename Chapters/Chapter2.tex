...
Paloalto's research unit provided a nice medium level overview of DNS tunneling\cite{tunneling2}. They explain how the protocol can be used either to exfiltrate or infiltrate data with infected machines. They show how different parts of the protocol can be exploited. Using the request queries names to embed information directly in the domain name queried with encoded data, using the type of RR to indicate the CnC the status or type of response expected, and finally, using the type of the response codes (NXDOMAINS or NOERROR) to define the malicious response.
...
IMAGE: DIAGRAM OF THE WORKFLOW (DONE)
...
generated by this algorithm. In fact, it is very difficult to interpret
the result and to identify the relationship between an input and its
target.
...
First, we will present the Exposure experiment, the features they extract and what model they have created. \\
...
\subsubsection{Exposure experiment}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Time-efficient local botnet infections}
...
The secomd part presented very interesting studies and projects that allowed the detection of the different evasion mechanisms using the DNS protocol. They allowed us to extract valuable features and techniques for each mechanism. 
...
for each type of mechanism:

{
    For the X mechanisms, we learned X and have obtained X features, that will be part of our experiment:
    #list + explanation of features for each evasion mechanism.
}
...
The objective is going to test out the features presented by all the papers mentioned and select the best combination for smaller environments. We want to avoid having too many features due to the "curse of dimensionality" \cite{curse}, which is caused by the amount of features for a model that increases its run-time exponentially. We also want to compare how a combined approach would do against a separate approach where we pipe 4-5  classifiers focused on a single evasion mechanism. 

Our suppositions are that the combined approach will probably do really good against new approaches and botnets using multiple techniques. But the pipeline of focused classifiers should perform better against botnets using only one type of evasion.