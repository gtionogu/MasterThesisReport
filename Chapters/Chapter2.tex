...
\paragraph{Example of the DNS protocol \cite{dns2}\cite{dns3}} This will later help us understand the abuses of the protocol .\\
...
\item \textbf{Standard Deviation of TTL} TODO: for a domain this is the average TTL compared to the mean TTL.\\
\item \textbf{Number of distinct TTL values}is the simple TTL count for the entry.\\
\item \textbf{Number of TTL change} TODO: is the different TTLs values for the same IP/domain \\
...
Features 1 to 4 have been left out because couldn't and a datasets
long in time enough to put in practice these features. Furthermore,
the CIC is a compilation of different labeled datasets which is equivalent
of not having timestamps.

There is also a possibility to analyze the DNS queries and not only the answers to uncovers techniques such as tunneling.


This type of project already exist such as the project Exposure detailed above. 
