...
%DGA PREVIOUS MODEL TOCHANGE
%For each evasion technique discussed we have picked some of the features and methodologies proposed that we are going to test and assess. For each complete approach we train the classier on 80\% of the dataset with a 10-kfold cross validation and test the predictions on the 20\% remaining. Then using the metrics below as well as visual plots we analyse the weight of the dierent features, correlation with the other features and possible combinations or improvements. The metrics used to assess the quality of the features are the following:

\includegraphics[scale=.6]{img/pers_ml_workflow.png}


%EXPOSURE PREVIOUS MODEL TOCHANGE
...
analysis. From there we assess the features through the classiers in
specic cases. The general case with all the features, then depending
on the correlations found in the previous step we assess groups of features.
For the classication process, we have taken 5 classiers that
have proven their potential to see if any has a better way to classify
the data. The classiers used in the DGA analysis were Logistic regression,
linear discriminant analysis, k-nearest neighbors, decision
tree classier and Naive Bayes. After training the classiers, we test
the out by predicting the evaluation set with the best classiers.
